...
class Customer(models.Model):
    ...
    def show_status(self):
        ...
        return mark_safe(
            '<span style="background-color:{};color: white;padding: 5px;">{}</span>'.format(color_dict.get(self.status),
...
class ClassList(models.Model):
    ...
    def __str__(self):
        return '{}-{}({})'.format(self.get_course_display(), self.semester, self.campuses.name)
...
class CourseRecord(models.Model):
    ...
    teacher = models.ForeignKey('UserProfile', verbose_name="讲师", related_name='t_course_records')
    recorder = models.ForeignKey('UserProfile', verbose_name="记录者", related_name='r_course_records')
